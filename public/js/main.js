...
function generateList(list) {
  ...
  for (const flair of list) {
    ...
    if (
      flair.flair === $("#first-flair").text() ||
      flair.flair === $("#second-flair").text()
    ) {
...
function registerEventListeners() {
  ...
  $("#save").on("click", (_) => {
    ...
    if (!firstFlairSelection && !secondFlairSelection) {
      swal.fire(
        "Error!",
        "Invalid flair selection! You must select at least one flair.",
        "error"
      );
      return;
    }
    ...
    $.ajax({
      ...
      success(data, textStatus, jqXHR) {
        swal.fire(
          "Success!",
          "Your flair should be updated in the next 15 minutes!",
          "success"
        );
      },
      error(jqXHR, textStatus) {
        if (jqXHR.status === 403) {
          ...
          swal.fire(
            "Error!",
            "Server error requesting profile information!",
            "error"
          );
        } else {
  ...
  $("#deleteFlair").on("click", (ev) => {
    $.ajax({
      ...
      success(data, textStatus, jqXHR) {
        swal.fire(
          "Success!",
          "Your flair should be deleted in the next 15 minutes!",
          "success"
        );
      },
      error(jqXHR, textStatus) {
